...
4. 在"Authorization"（授权）标签页中选择验证方式（HTTP或DNS）
5. 在"Deployment"（部署）标签页中选择部署方式
6. 点击"请求证书"按钮获取证书

### 配置Next.js使用HTTPS

获取证书后，需要配置Next.js应用使用HTTPS。我们提供了一个简单的解决方案，可以与现有的HTTP服务共存：

1. 将以下文件复制到服务器的JuJ Hub目录：
   - `https-server.js`：HTTPS服务器配置
   - `ecosystem-https.config.js`：PM2配置文件
   - `setup-https.ps1`：设置脚本
...
2. **ecosystem-https.config.js**：
   - PM2配置文件，定义两个应用：
     - jujhub-http：原有的HTTP服务（3000端口）
     - jujhub-https：新增的HTTPS服务（3001端口）
...
HTTPS配置使用以下端口转发规则：
- 80端口 → 3000端口（HTTP服务）
- 443端口 → 3001端口（HTTPS服务）

### 证书自动续期

Certify The Web会自动处理证书续期，无需手动操作。确保：
1. Certify The Web设置为开机自启
2. 自动续期功能已启用
